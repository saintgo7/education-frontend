...
/**
 * Composable Example 4
 * Reusable composition function
 */

export interface UseExample4Options {
  initialValue?: number;
  step?: number;
  max?: number;
  min?: number;
...
export function useExample4(options: UseExample4Options = {}) {
  ...
  watch(value, (newVal) => {
    console.log(`useExample4: value changed to ${newVal}`);
  });

  onMounted(() => {
    console.log('useExample4 mounted');
  });

  onUnmounted(() => {
    console.log('useExample4 unmounted');
  });
