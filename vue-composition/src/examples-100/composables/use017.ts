...
/**
 * Composable Example 17
 * Reusable composition function
 */

export interface UseExample17Options {
  initialValue?: number;
  step?: number;
  max?: number;
  min?: number;
...
export function useExample17(options: UseExample17Options = {}) {
  ...
  watch(value, (newVal) => {
    console.log(`useExample17: value changed to ${newVal}`);
  });

  onMounted(() => {
    console.log('useExample17 mounted');
  });

  onUnmounted(() => {
    console.log('useExample17 unmounted');
  });
