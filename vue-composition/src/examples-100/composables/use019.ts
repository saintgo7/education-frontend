...
/**
 * Composable Example 19
 * Reusable composition function
 */

export interface UseExample19Options {
  initialValue?: number;
  step?: number;
  max?: number;
  min?: number;
...
export function useExample19(options: UseExample19Options = {}) {
  ...
  watch(value, (newVal) => {
    console.log(`useExample19: value changed to ${newVal}`);
  });

  onMounted(() => {
    console.log('useExample19 mounted');
  });

  onUnmounted(() => {
    console.log('useExample19 unmounted');
  });
