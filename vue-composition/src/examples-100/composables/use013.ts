...
/**
 * Composable Example 13
 * Reusable composition function
 */

export interface UseExample13Options {
  initialValue?: number;
  step?: number;
  max?: number;
  min?: number;
...
export function useExample13(options: UseExample13Options = {}) {
  ...
  watch(value, (newVal) => {
    console.log(`useExample13: value changed to ${newVal}`);
  });

  onMounted(() => {
    console.log('useExample13 mounted');
  });

  onUnmounted(() => {
    console.log('useExample13 unmounted');
  });
