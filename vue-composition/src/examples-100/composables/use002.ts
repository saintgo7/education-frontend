...
/**
 * Composable Example 2
 * Reusable composition function
 */

export interface UseExample2Options {
  initialValue?: number;
  step?: number;
  max?: number;
  min?: number;
...
export function useExample2(options: UseExample2Options = {}) {
  ...
  watch(value, (newVal) => {
    console.log(`useExample2: value changed to ${newVal}`);
  });

  onMounted(() => {
    console.log('useExample2 mounted');
  });

  onUnmounted(() => {
    console.log('useExample2 unmounted');
  });
