...
/**
 * Composable Example 14
 * Reusable composition function
 */

export interface UseExample14Options {
  initialValue?: number;
  step?: number;
  max?: number;
  min?: number;
...
export function useExample14(options: UseExample14Options = {}) {
  ...
  watch(value, (newVal) => {
    console.log(`useExample14: value changed to ${newVal}`);
  });

  onMounted(() => {
    console.log('useExample14 mounted');
  });

  onUnmounted(() => {
    console.log('useExample14 unmounted');
  });
