...
/**
 * Composable Example 20
 * Reusable composition function
 */

export interface UseExample20Options {
  initialValue?: number;
  step?: number;
  max?: number;
  min?: number;
...
export function useExample20(options: UseExample20Options = {}) {
  ...
  watch(value, (newVal) => {
    console.log(`useExample20: value changed to ${newVal}`);
  });

  onMounted(() => {
    console.log('useExample20 mounted');
  });

  onUnmounted(() => {
    console.log('useExample20 unmounted');
  });
