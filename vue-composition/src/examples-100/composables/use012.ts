...
/**
 * Composable Example 12
 * Reusable composition function
 */

export interface UseExample12Options {
  initialValue?: number;
  step?: number;
  max?: number;
  min?: number;
...
export function useExample12(options: UseExample12Options = {}) {
  ...
  watch(value, (newVal) => {
    console.log(`useExample12: value changed to ${newVal}`);
  });

  onMounted(() => {
    console.log('useExample12 mounted');
  });

  onUnmounted(() => {
    console.log('useExample12 unmounted');
  });
