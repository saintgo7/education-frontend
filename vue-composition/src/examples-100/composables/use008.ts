...
/**
 * Composable Example 8
 * Reusable composition function
 */

export interface UseExample8Options {
  initialValue?: number;
  step?: number;
  max?: number;
  min?: number;
...
export function useExample8(options: UseExample8Options = {}) {
  ...
  watch(value, (newVal) => {
    console.log(`useExample8: value changed to ${newVal}`);
  });

  onMounted(() => {
    console.log('useExample8 mounted');
  });

  onUnmounted(() => {
    console.log('useExample8 unmounted');
  });
