...
/**
 * Composable Example 11
 * Reusable composition function
 */

export interface UseExample11Options {
  initialValue?: number;
  step?: number;
  max?: number;
  min?: number;
...
export function useExample11(options: UseExample11Options = {}) {
  ...
  watch(value, (newVal) => {
    console.log(`useExample11: value changed to ${newVal}`);
  });

  onMounted(() => {
    console.log('useExample11 mounted');
  });

  onUnmounted(() => {
    console.log('useExample11 unmounted');
  });
