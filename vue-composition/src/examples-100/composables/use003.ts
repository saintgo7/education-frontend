...
/**
 * Composable Example 3
 * Reusable composition function
 */

export interface UseExample3Options {
  initialValue?: number;
  step?: number;
  max?: number;
  min?: number;
...
export function useExample3(options: UseExample3Options = {}) {
  ...
  watch(value, (newVal) => {
    console.log(`useExample3: value changed to ${newVal}`);
  });

  onMounted(() => {
    console.log('useExample3 mounted');
  });

  onUnmounted(() => {
    console.log('useExample3 unmounted');
  });
