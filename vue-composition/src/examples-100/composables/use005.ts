...
/**
 * Composable Example 5
 * Reusable composition function
 */

export interface UseExample5Options {
  initialValue?: number;
  step?: number;
  max?: number;
  min?: number;
...
export function useExample5(options: UseExample5Options = {}) {
  ...
  watch(value, (newVal) => {
    console.log(`useExample5: value changed to ${newVal}`);
  });

  onMounted(() => {
    console.log('useExample5 mounted');
  });

  onUnmounted(() => {
    console.log('useExample5 unmounted');
  });
