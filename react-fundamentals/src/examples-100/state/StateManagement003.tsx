...
interface StateContext3 {
  state: State3;
  dispatch: React.Dispatch<Action3>;
  actions: {
...
const StateContext3 = createContext<StateContext3 | undefined>(undefined);

export function StateProvider3({ children }: { children: ReactNode }) {
  ...
  return (
    <StateContext3.Provider value={{ state, dispatch, actions }}>
      {children}
    </StateContext3.Provider>
  );
}

export function useStateManagement3() {
  const context = useContext(StateContext3);
  if (!context) {
    throw new Error('useStateManagement3 must be used within StateProvider3');
  }
  return context;
}

export function StateManagementDemo3() {
  const { state, actions } = useStateManagement3();
  ...
  return (
    <div className="state-demo-3">
      <h3>State Management Example 3</h3>
...
export default StateManagementDemo3;
