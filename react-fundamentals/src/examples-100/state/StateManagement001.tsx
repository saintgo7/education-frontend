...
interface StateContext1 {
  state: State1;
  dispatch: React.Dispatch<Action1>;
  actions: {
...
const StateContext1 = createContext<StateContext1 | undefined>(undefined);

export function StateProvider1({ children }: { children: ReactNode }) {
  ...
  return (
    <StateContext1.Provider value={{ state, dispatch, actions }}>
      {children}
    </StateContext1.Provider>
  );
}

export function useStateManagement1() {
  const context = useContext(StateContext1);
  if (!context) {
    throw new Error('useStateManagement1 must be used within StateProvider1');
  }
  return context;
}

export function StateManagementDemo1() {
  const { state, actions } = useStateManagement1();
  ...
  return (
    <div className="state-demo-1">
      <h3>State Management Example 1</h3>
...
export default StateManagementDemo1;
