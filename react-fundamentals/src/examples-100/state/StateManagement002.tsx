...
interface StateContext2 {
  state: State2;
  dispatch: React.Dispatch<Action2>;
  actions: {
...
const StateContext2 = createContext<StateContext2 | undefined>(undefined);

export function StateProvider2({ children }: { children: ReactNode }) {
  ...
  return (
    <StateContext2.Provider value={{ state, dispatch, actions }}>
      {children}
    </StateContext2.Provider>
  );
}

export function useStateManagement2() {
  const context = useContext(StateContext2);
  if (!context) {
    throw new Error('useStateManagement2 must be used within StateProvider2');
  }
  return context;
}

export function StateManagementDemo2() {
  const { state, actions } = useStateManagement2();
  ...
  return (
    <div className="state-demo-2">
      <h3>State Management Example 2</h3>
...
export default StateManagementDemo2;
