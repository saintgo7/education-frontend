...
export function API4() {
  const [users, setUsers] = useState<APIState4<User4[]>>({
    data: null,
    loading: false,
    error: null,
  });

  const [selectedUser, setSelectedUser] = useState<APIState4<User4>>({
    data: null,
    loading: false,
    error: null,
  });
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      const mockUsers: User4[] = Array.from({ length: 10 }, (_, i) => ({
        id: i + 1,
        name: `User ${i + 1}`,
