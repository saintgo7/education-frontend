...
export function API1() {
  const [users, setUsers] = useState<APIState1<User1[]>>({
    data: null,
    loading: false,
    error: null,
  });

  const [selectedUser, setSelectedUser] = useState<APIState1<User1>>({
    data: null,
    loading: false,
    error: null,
  });
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      const mockUsers: User1[] = Array.from({ length: 10 }, (_, i) => ({
        id: i + 1,
        name: `User ${i + 1}`,
