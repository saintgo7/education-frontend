...
export function API3() {
  const [users, setUsers] = useState<APIState3<User3[]>>({
    data: null,
    loading: false,
    error: null,
  });

  const [selectedUser, setSelectedUser] = useState<APIState3<User3>>({
    data: null,
    loading: false,
    error: null,
  });
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      const mockUsers: User3[] = Array.from({ length: 10 }, (_, i) => ({
        id: i + 1,
        name: `User ${i + 1}`,
