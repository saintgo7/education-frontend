...
export function API5() {
  const [users, setUsers] = useState<APIState5<User5[]>>({
    data: null,
    loading: false,
    error: null,
  });

  const [selectedUser, setSelectedUser] = useState<APIState5<User5>>({
    data: null,
    loading: false,
    error: null,
  });
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      const mockUsers: User5[] = Array.from({ length: 10 }, (_, i) => ({
        id: i + 1,
        name: `User ${i + 1}`,
