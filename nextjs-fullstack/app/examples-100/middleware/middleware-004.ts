...
export function middleware4(request: NextRequest) {
  ...
  // Example 4: Various middleware patterns

  // 1. Authentication check
  const authToken = request.cookies.get('auth-token')?.value;
  if (pathname.startsWith('/protected-4') && !authToken) {
  ...
  // 3. Custom headers
  response.headers.set('X-Middleware-4', 'processed');
  ...
  response.cookies.set('ab-test-4', variant, {
    httpOnly: true,
    secure: true,
    sameSite: 'strict',
  });
  ...
  // 6. Request logging
  console.log(`[Middleware 4] ${request.method} ${pathname}`);

  // 7. Query parameter validation
  const requiredParam = searchParams.get('required');
  if (pathname.startsWith('/api/example-4') && !requiredParam) {
