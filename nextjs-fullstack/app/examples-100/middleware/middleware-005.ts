...
export function middleware5(request: NextRequest) {
  const { pathname, searchParams } = request.nextUrl;
  const response = NextResponse.next();

  // Example 5: Various middleware patterns

  // 1. Authentication check
  const authToken = request.cookies.get('auth-token')?.value;
  if (pathname.startsWith('/protected-5') && !authToken) {
  ...
  // 3. Custom headers
  response.headers.set('X-Middleware-5', 'processed');
  ...
  response.cookies.set('ab-test-5', variant, {
    httpOnly: true,
    secure: true,
    sameSite: 'strict',
  });
  ...
  // 6. Request logging
  console.log(`[Middleware 5] ${request.method} ${pathname}`);

  // 7. Query parameter validation
  const requiredParam = searchParams.get('required');
  if (pathname.startsWith('/api/example-5') && !requiredParam) {
