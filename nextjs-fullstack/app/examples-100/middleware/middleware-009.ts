...
export function middleware9(request: NextRequest) {
  const { pathname, searchParams } = request.nextUrl;
  const response = NextResponse.next();

  // Example 9: Various middleware patterns

  // 1. Authentication check
  const authToken = request.cookies.get('auth-token')?.value;
  if (pathname.startsWith('/protected-9') && !authToken) {
  ...
  // 3. Custom headers
  response.headers.set('X-Middleware-9', 'processed');
  ...
  response.cookies.set('ab-test-9', variant, {
    httpOnly: true,
    secure: true,
    sameSite: 'strict',
  });
  ...
  // 6. Request logging
  console.log(`[Middleware 9] ${request.method} ${pathname}`);

  // 7. Query parameter validation
  const requiredParam = searchParams.get('required');
  if (pathname.startsWith('/api/example-9') && !requiredParam) {
