...
export function middleware2(request: NextRequest) {
  ...
  // Example 2: Various middleware patterns

  // 1. Authentication check
  const authToken = request.cookies.get('auth-token')?.value;
  if (pathname.startsWith('/protected-2') && !authToken) {
  ...
  // 3. Custom headers
  response.headers.set('X-Middleware-2', 'processed');
  ...
  response.cookies.set('ab-test-2', variant, {
    httpOnly: true,
    secure: true,
    sameSite: 'strict',
  });
  ...
  // 6. Request logging
  console.log(`[Middleware 2] ${request.method} ${pathname}`);

  // 7. Query parameter validation
  const requiredParam = searchParams.get('required');
  if (pathname.startsWith('/api/example-2') && !requiredParam) {
