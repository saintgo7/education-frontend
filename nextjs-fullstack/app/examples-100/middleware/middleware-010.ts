...
export function middleware10(request: NextRequest) {
  const { pathname, searchParams } = request.nextUrl;
  const response = NextResponse.next();

  // Example 10: Various middleware patterns

  // 1. Authentication check
  const authToken = request.cookies.get('auth-token')?.value;
  if (pathname.startsWith('/protected-10') && !authToken) {
  ...
  // 3. Custom headers
  response.headers.set('X-Middleware-10', 'processed');
  ...
  response.cookies.set('ab-test-10', variant, {
    httpOnly: true,
    secure: true,
    sameSite: 'strict',
  });
  ...
  // 6. Request logging
  console.log(`[Middleware 10] ${request.method} ${pathname}`);

  // 7. Query parameter validation
  const requiredParam = searchParams.get('required');
  if (pathname.startsWith('/api/example-10') && !requiredParam) {
