...
export function middleware1(request: NextRequest) {
  ...
  // Example 1: Various middleware patterns

  // 1. Authentication check
  const authToken = request.cookies.get('auth-token')?.value;
  if (pathname.startsWith('/protected-1') && !authToken) {
  ...
  // 3. Custom headers
  response.headers.set('X-Middleware-1', 'processed');
  ...
  response.cookies.set('ab-test-1', variant, {
    httpOnly: true,
    secure: true,
    sameSite: 'strict',
  });
  ...
  // 6. Request logging
  console.log(`[Middleware 1] ${request.method} ${pathname}`);

  // 7. Query parameter validation
  const requiredParam = searchParams.get('required');
  if (pathname.startsWith('/api/example-1') && !requiredParam) {
