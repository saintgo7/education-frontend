...
export function middleware3(request: NextRequest) {
  ...
  // Example 3: Various middleware patterns

  // 1. Authentication check
  const authToken = request.cookies.get('auth-token')?.value;
  if (pathname.startsWith('/protected-3') && !authToken) {
  ...
  // 3. Custom headers
  response.headers.set('X-Middleware-3', 'processed');
  ...
  response.cookies.set('ab-test-3', variant, {
    httpOnly: true,
    secure: true,
    sameSite: 'strict',
  });
  ...
  // 6. Request logging
  console.log(`[Middleware 3] ${request.method} ${pathname}`);

  // 7. Query parameter validation
  const requiredParam = searchParams.get('required');
  if (pathname.startsWith('/api/example-3') && !requiredParam) {
