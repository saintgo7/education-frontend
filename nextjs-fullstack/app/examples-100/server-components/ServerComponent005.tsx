...
async function fetchData5(): Promise<Data5[]> {
  // Simulate API call
  ...
  return Array.from({ length: 10 }, (_, index) => ({
    id: 5 * 100 + index,
    title: `Item ${index + 1}`,
    content: `This is content for server component 5, item ${index + 1}`,
    timestamp: Date.now(),
  }));
}

async function ServerDataDisplay5() {
  const data = await fetchData5();
  const headersList = headers();
  const userAgent = headersList.get('user-agent') || 'Unknown';

  return (
    <div className="server-data-5">
...
export default async function ServerComponent5() {
  ...
  return (
    <div className="server-component-5" data-theme={theme}>
      <h2>Server Component Example 5</h2>
