...
async function fetchData7(): Promise<Data7[]> {
  // Simulate API call
  ...
  return Array.from({ length: 10 }, (_, index) => ({
    id: 7 * 100 + index,
    title: `Item ${index + 1}`,
    content: `This is content for server component 7, item ${index + 1}`,
    timestamp: Date.now(),
  }));
}

async function ServerDataDisplay7() {
  const data = await fetchData7();
  const headersList = headers();
  const userAgent = headersList.get('user-agent') || 'Unknown';

  return (
    <div className="server-data-7">
...
export default async function ServerComponent7() {
  const cookieStore = cookies();
  const theme = cookieStore.get('theme')?.value || 'light';

  return (
    <div className="server-component-7" data-theme={theme}>
      <h2>Server Component Example 7</h2>
