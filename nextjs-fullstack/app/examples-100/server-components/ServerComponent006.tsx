...
async function fetchData6(): Promise<Data6[]> {
  // Simulate API call
  ...
  return Array.from({ length: 10 }, (_, index) => ({
    id: 6 * 100 + index,
    title: `Item ${index + 1}`,
    content: `This is content for server component 6, item ${index + 1}`,
    timestamp: Date.now(),
  }));
}

async function ServerDataDisplay6() {
  const data = await fetchData6();
  const headersList = headers();
  const userAgent = headersList.get('user-agent') || 'Unknown';

  return (
    <div className="server-data-6">
...
export default async function ServerComponent6() {
  const cookieStore = cookies();
  const theme = cookieStore.get('theme')?.value || 'light';

  return (
    <div className="server-component-6" data-theme={theme}>
      <h2>Server Component Example 6</h2>
