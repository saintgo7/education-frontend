...
async function fetchData2(): Promise<Data2[]> {
  // Simulate API call
  ...
  return Array.from({ length: 10 }, (_, index) => ({
    id: 2 * 100 + index,
    title: `Item ${index + 1}`,
    content: `This is content for server component 2, item ${index + 1}`,
    timestamp: Date.now(),
  }));
}

async function ServerDataDisplay2() {
  const data = await fetchData2();
  const headersList = headers();
  const userAgent = headersList.get('user-agent') || 'Unknown';

  return (
    <div className="server-data-2">
...
export default async function ServerComponent2() {
  ...
  return (
    <div className="server-component-2" data-theme={theme}>
      <h2>Server Component Example 2</h2>
