...
async function fetchData4(): Promise<Data4[]> {
  // Simulate API call
  ...
  return Array.from({ length: 10 }, (_, index) => ({
    id: 4 * 100 + index,
    title: `Item ${index + 1}`,
    content: `This is content for server component 4, item ${index + 1}`,
    timestamp: Date.now(),
  }));
}

async function ServerDataDisplay4() {
  const data = await fetchData4();
  const headersList = headers();
  const userAgent = headersList.get('user-agent') || 'Unknown';

  return (
    <div className="server-data-4">
...
export default async function ServerComponent4() {
  ...
  return (
    <div className="server-component-4" data-theme={theme}>
      <h2>Server Component Example 4</h2>
