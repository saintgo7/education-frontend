...
async function fetchData3(): Promise<Data3[]> {
  // Simulate API call
  ...
  return Array.from({ length: 10 }, (_, index) => ({
    id: 3 * 100 + index,
    title: `Item ${index + 1}`,
    content: `This is content for server component 3, item ${index + 1}`,
    timestamp: Date.now(),
  }));
}

async function ServerDataDisplay3() {
  const data = await fetchData3();
  const headersList = headers();
  const userAgent = headersList.get('user-agent') || 'Unknown';

  return (
    <div className="server-data-3">
...
export default async function ServerComponent3() {
  ...
  return (
    <div className="server-component-3" data-theme={theme}>
      <h2>Server Component Example 3</h2>
