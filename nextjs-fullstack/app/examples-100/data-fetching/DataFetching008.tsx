...
const getCachedData8 = cache(async (id: number) => {
  console.log(`Fetching data 8 for id: ${id}`);
...
const getStableData8 = unstable_cache(
...
async function getParallelData8() {
...
async function getSequentialData8(userId: number) {
...
async function StreamedData8() {
  const data = await getCachedData8(8);
...
export default async function DataFetching8() {
  // Multiple data fetching patterns
  const cachedData = await getCachedData8(8);
  const stableData = await getStableData8('category-8');

  return (
    <div className="data-fetching-8">
      <h2>Data Fetching Example 8</h2>
