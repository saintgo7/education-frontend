...
const getCachedData6 = cache(async (id: number) => {
  console.log(`Fetching data 6 for id: ${id}`);
...
const getStableData6 = unstable_cache(
...
async function getParallelData6() {
...
async function getSequentialData6(userId: number) {
...
async function StreamedData6() {
  const data = await getCachedData6(6);
...
export default async function DataFetching6() {
  // Multiple data fetching patterns
  const cachedData = await getCachedData6(6);
  const stableData = await getStableData6('category-6');

  return (
    <div className="data-fetching-6">
      <h2>Data Fetching Example 6</h2>
