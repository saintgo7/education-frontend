...
const getCachedData3 = cache(async (id: number) => {
  console.log(`Fetching data 3 for id: ${id}`);
...
const getStableData3 = unstable_cache(
...
async function getParallelData3() {
...
async function getSequentialData3(userId: number) {
...
async function StreamedData3() {
  const data = await getCachedData3(3);
...
export default async function DataFetching3() {
  // Multiple data fetching patterns
  const cachedData = await getCachedData3(3);
  const stableData = await getStableData3('category-3');

  return (
    <div className="data-fetching-3">
      <h2>Data Fetching Example 3</h2>
